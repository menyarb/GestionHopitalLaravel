<template>
    <div class="content">
      ...
      <div class="col-12">
                        <div class="bg-light rounded h-100 p-4">
                            <h6 class="mb-4">Liste user Table</h6>
                            <div class="table-responsive">
                              <router-link  to="/add_user"><button type="button" class="btn btn-outline-primary m-2 ms-auto"><i class="fa fa-home me-2"></i>add user</button></router-link>
  ...
  <tr v-for="user in users" :key="user.id">
    ...
    <td>
    <RouterLink :to="{path:'/modify_user/'+user.id+''}" class="btn btn-success m-2">Modify</RouterLink>
  ...
  setup() {
    ...
    const modifyUser = async (id) => {
      try {
        ...
        router.push({ name: 'modify_user', params: { id: user.id } });
      } catch (error) {
        console.error(error);
      }
    };
